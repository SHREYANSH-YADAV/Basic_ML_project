...
dup_count = df.duplicated(subset=[c for c in df.columns if c != 'EmployeeID']).sum()
print("\nDuplicate rows:", dup_count)
df = df.drop_duplicates(subset=[c for c in df.columns if c != 'EmployeeID']).reset_index(drop=True)
print("Shape after removing duplicates:", df.shape)

numeric_cols = ['Age', 'MonthlyIncome', 'DistanceFromHome_KM', 'YearsAtCompany', 'JobSatisfaction', 'WorkLifeBalance', 'EnvironmentSatisfaction', 'TrainingTimesLastYear']
categorical_cols = ['Gender', 'Department', 'JobRole', 'OverTime', 'MaritalStatus']
...
plt.title("Target Class Balance: Attrition")
plt.show()
...
corr = df[numeric_cols + (["Attrition"] if 'classification'=='regression' else [])].corr()
...
plt.scatter(df[numeric_cols[0]], df['Attrition'] if 'classification'=='regression' else df[numeric_cols[1]], alpha=0.4, color="teal")
plt.xlabel(numeric_cols[0])
plt.ylabel('Attrition' if 'classification'=='regression' else numeric_cols[1])
plt.title(f"Scatter: {numeric_cols[0]} vs " + ('Attrition' if 'classification'=='regression' else numeric_cols[1]))
plt.show()
...
df['Satisfaction_Index'] = (df['JobSatisfaction'] + df['WorkLifeBalance'] + df['EnvironmentSatisfaction']) / 3
numeric_cols = numeric_cols + ['Income_per_YearAtCompany', 'Satisfaction_Index']
print("Engineered features added:", ['Income_per_YearAtCompany', 'Satisfaction_Index'])
...
joblib.dump(best_pipe, "Model/employee_attrition_model.pkl")
metadata = {
...
with open("Model/employee_attrition_metadata.json", "w") as f:
