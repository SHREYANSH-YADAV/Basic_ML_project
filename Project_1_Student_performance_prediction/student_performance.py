...
dup_count = df.duplicated(subset=[c for c in df.columns if c != 'StudentID']).sum()
print("\nDuplicate rows:", dup_count)
df = df.drop_duplicates(subset=[c for c in df.columns if c != 'StudentID']).reset_index(drop=True)
print("Shape after removing duplicates:", df.shape)

numeric_cols = ['Attendance_Percent', 'Weekly_Study_Hours', 'Assignment_Score', 'Internal_Assessment_1', 'Internal_Assessment_2', 'Previous_GPA', 'Online_Study_Hours']
categorical_cols = ['Gender', 'Extracurricular_Activities', 'Parental_Education', 'Internet_Access']
...
sns.histplot(df['Final_Exam_Score'], kde=True, color="darkorange")
plt.title("Target Distribution: Final_Exam_Score")
plt.show()
...
corr = df[numeric_cols + (["Final_Exam_Score"] if 'regression'=='regression' else [])].corr()
...
plt.scatter(df[numeric_cols[0]], df['Final_Exam_Score'] if 'regression'=='regression' else df[numeric_cols[1]], alpha=0.4, color="teal")
plt.xlabel(numeric_cols[0])
plt.ylabel('Final_Exam_Score' if 'regression'=='regression' else numeric_cols[1])
plt.title(f"Scatter: {numeric_cols[0]} vs " + ('Final_Exam_Score' if 'regression'=='regression' else numeric_cols[1]))
plt.show()

# ## Phase 5: Feature Engineering
# Create new meaningful features, encode categoricals, and select important features.

df['Avg_Internal_Assessment'] = (df['Internal_Assessment_1'] + df['Internal_Assessment_2']) / 2
df['Total_Study_Hours'] = df['Weekly_Study_Hours'] + df['Online_Study_Hours']
numeric_cols = numeric_cols + ['Avg_Internal_Assessment', 'Total_Study_Hours']
print("Engineered features added:", ['Avg_Internal_Assessment', 'Total_Study_Hours'])
...
joblib.dump(best_pipe, "Model/student_performance_model.pkl")
metadata = {
